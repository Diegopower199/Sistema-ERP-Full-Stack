...
public interface TipoUsuarioRepository extends JpaRepository<TipoUsuarioModel, Integer> {
    ...
    @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM TipoUsuarioModel c WHERE c.permiso_usuario = :id_permiso_usuario")
    boolean existsByPermiso_usuario(@Param("id_permiso_usuario") PermisoUsuarioModel id_permiso_usuario);

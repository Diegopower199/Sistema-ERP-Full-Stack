...
export default function FormDetallesFacturasClientes({
  toggleForm,
  detalleFacturaClienteDataForm,
  formUpdateTrigger,
  operationType,
}) {
  const generoOptions = [
    ...
    const fetchData = async () => {
      try {
        ...
        if (operationType === "update" || operationType === "view") {
          if (validarFechaYYYYMMDD(detalleFacturaClienteDataForm.fecha_nacimiento) === null) {
            const fechaNacimientoFormateada = formatearFechaYYYYMMDD(
              detalleFacturaClienteDataForm.fecha_nacimiento
            );
